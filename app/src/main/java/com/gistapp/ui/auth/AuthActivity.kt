...
/**
 * Halaman login — user memasukkan GitHub Personal Access Token.
 * Setelah diverifikasi, lanjut ke MainActivity.
 */
class AuthActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.tvSkip.setOnClickListener {
            // Mode publik: tanpa token, hanya lihat public gists
            navigateToMain()
        }

        binding.tvHowToGetToken.setOnClickListener {
            binding.tvTokenHint.visibility =
                if (binding.tvTokenHint.visibility == View.VISIBLE) View.GONE
                else View.VISIBLE
        }
    }
    ...
    private fun verifyAndLogin(token: String) {
        ...
        lifecycleScope.launch {
            ...
            result.onSuccess { user ->
                Toast.makeText(
                    this@AuthActivity,
                    "Login berhasil! Selamat datang, ${user.login}",
                    Toast.LENGTH_SHORT
                ).show()
                navigateToMain()
            }.onFailure { error ->
